...
class ToTensor:
    ...
    def __call__(self, sample):
        ...
        for key in [k for k in sample.keys() if k != "mask"]:
            sample[key] = TF.to_tensor(sample[key].astype(np.float32) / 255)
        return sample
...
def test_augm(sample,size=512):
    augms = [ A.Resize(height=size, width=size, p=1.0) ,A.HorizontalFlip(p=0.1)]
    return A.Compose(augms)(image=sample["image"], mask=sample["mask"])
